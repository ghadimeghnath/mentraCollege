...
export function LoginForm({ expectedRole }: { expectedRole: string }) {
  ...
  const handleGoogleSignIn = () => {
    setIsLoading(true);
    signIn("google", { callbackUrl: `/${expectedRole.toLowerCase()}/` });
  };
    ...
    <div className="w-full max-w-md p-8 space-y-8 bg-card/80 backdrop-blur-md text-card-foreground shadow-2xl rounded-2xl border border-white/10 dark:border-white/5">
      <div className="space-y-2 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {isAdmin ? "Use your Google account or email and password to access your account." : "Use your @vvm.edu.in institutional Google account."}
        </p>
      </div>
